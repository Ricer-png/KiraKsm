...
class BestdoriScraperArea:
    def __init__(self, target_name, log_queue, progress_queue, stop_flag, wav_flag):
        ...
        if self.kyara_name:
            ...
            if self.wav_flag.is_set():
                self.save_card_audio_folder = os.path.join(self.save_folder, f"{self.kyara_name[2]}_wav")
            else:
    ...
    # select
    def select_kyara(self, driver):
        # band story
        band_story_tab = driver.find_element(By.XPATH, "//a[contains(text(), 'Area Conversation')]")
        band_story_tab.click()
        time.sleep(0.4)

        # select kyara   1.all-button  2. kyara
    ...
    def extract_conv_ids(self, driver):
        while True:
            try:
        ...
        return re.findall(r'/tool/storyviewer/area/jp/(\d+)(?:/|$)', page_source)

    def load_log(self):
        log_path = os.path.join(self.save_folder, "log_area.json")
    ...
    def scrape_card_audio(self, driver, card_ID):
        driver.get(f"https://bestdori.com/tool/storyviewer/area/jp/{card_ID}")
        ...
        for block in dialogue_blocks:
            ...
            if name in self.kyara_name:
                ...
                if audio_url:
                    ...
                    if self.wav_flag.is_set():
                        file_path = os.path.join(self.save_card_audio_folder, f"{file_name}.wav")
                    else:
    ...
    def start_scraping(self):
        ...
        if not self.log_data:
            self.select_kyara(driver)
            card_IDs = self.extract_conv_ids(driver)
        ...
        if card_IDs:
            ...
            driver.quit()
            self.log_queue.put(f"已成功收集 {self.kyara_name[1]} 所有区域故事语音")
        else:
            self.log_queue.put(f"未找到 {self.kyara_name[1]} 有关的区域故事音频数据")
            driver.quit()
            exit()

def run_scraper_area(target_name, log_queue, progress_queue, stop_flag, wav_flag):
    scraper = BestdoriScraperArea(target_name, log_queue, progress_queue, stop_flag, wav_flag)
